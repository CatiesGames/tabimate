...
- {"op":"set_leg","fromStopId":"...","mode":"transit","durationMin?":25,"departureTime?":"14:03","arrivalTime?":"14:28","transit?":{"summary":"JR山手線","steps":[...],"fare":"¥170"},"notes?":"...","bookingType?":"ticket_required","bookingStatus?":"not_booked","booking?":{"url":"...","deadline?":"YYYY-MM-DD","note?":"..."}} 交通段掛在出發地點上,目的地自動是下一站。mode:walk|transit|drive|taxi|bike|flight|other。steps 每段:{"mode":"walk|transit|...","line?":"JR山手線","departureStop?":"澀谷","arrivalStop?":"新宿","departureTime?":"HH:MM","arrivalTime?":"HH:MM","headsign?":"...","numStops?":3} — 每段至少要有 line 或起訖站或時間,**不要寫只有 mode 的空段**(介面顯示不出來);單純一段路線用 summary 就好,steps 留給轉車。**新幹線/機場快線/特急指定席等要先買票的交通,設 bookingType(通常 ticket_required)並附訂票連結**,會出現在預約總覽與 PDF 清單
